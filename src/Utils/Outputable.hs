...
import Text.Megaparsec qualified as MParsec
...
data CodeStyle
  = -- | Still don't know if we're targetting C or Java.
    CStyle
  | JavaStyle
...
mkUserStyle settings depth
  ...
  where
    c
      | shouldUseColor settings = Colored
      | otherwise = Uncolored
...
cdocDeeperList f ds
  ...
  where
    worker ctx@CDocContext{cdocStyle = UserStyle (PartialDepth n) c}
...
isOperatorDoc d =
  let str = output d
   in not (null str) && isSymbol (head str)
...
instance
  (Outputable a, Outputable b, Outputable c, Outputable d, Outputable e)
...
instance
  ...
  , MParsec.VisualStream s
  , MParsec.TraversableStream s
  )
  => Outputable (MParsec.ParseErrorBundle s e)
  where
