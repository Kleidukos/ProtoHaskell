...
instance (Pretty e) => Pretty (UniqueMap e) where
  pretty = prettyUniqueMap pretty

prettyUniqueMap :: (e -> Doc ann) -> UniqueMap e -> Doc ann
prettyUniqueMap prettyE um =
  brackets $
    sep $
      punctuate
        comma
        [ pretty uniq <+> ":->" <+> prettyE e
        | (uniq, e) <- toListUM um
        ]
