...
foldBag
  :: (r -> r -> r) -- Replace TwoBags with this; should be associative
...
mapMaybeBag f (UnitBag x) = maybe EmptyBag UnitBag (f x)
...
mapBagM f (UnitBag x) = do
  r <- f x
  return (UnitBag r)
mapBagM f (TwoBags b1 b2) = do
  r1 <- mapBagM f b1
  r2 <- mapBagM f b2
  return (TwoBags r1 r2)
mapBagM f (ListBag xs) = do
  rs <- mapM f xs
  return (ListBag rs)
...
flatMapBagM f (TwoBags b1 b2) = do
  r1 <- flatMapBagM f b1
  r2 <- flatMapBagM f b2
...
flatMapBagPairM f (TwoBags b1 b2) = do
...
mapAndUnzipBagM f (UnitBag x) = do
  (r, s) <- f x
  return (UnitBag r, UnitBag s)
mapAndUnzipBagM f (TwoBags b1 b2) = do
  (r1, s1) <- mapAndUnzipBagM f b1
  (r2, s2) <- mapAndUnzipBagM f b2
  return (TwoBags r1 r2, TwoBags s1 s2)
mapAndUnzipBagM f (ListBag xs) = do
  ts <- mapM f xs
  let (rs, ss) = unzip ts
...
mapAccumBagL f s (TwoBags b1 b2) =
  let (s1, b1') = mapAccumBagL f s b1
      (s2, b2') = mapAccumBagL f s1 b2
   in (s2, TwoBags b1' b2')
mapAccumBagL f s (ListBag xs) =
  let (s', xs') = mapAccumL f s xs
   in (s', ListBag xs')
...
mapAccumBagLM f s (TwoBags b1 b2) = do
  (s1, b1') <- mapAccumBagLM f s b1
  (s2, b2') <- mapAccumBagLM f s1 b2
  return (s2, TwoBags b1' b2')
mapAccumBagLM f s (ListBag xs) = do
  (s', xs') <- mapAccumLM f s xs
  return (s', ListBag xs')
